...
def user_info(i,ck):
    ...
    if signid == []:
        print (f'账号【{str(i+1)}】初始化失败,请检测CK({ck["ck"]})是否正确!')
        return False
    else:
        ...
        if result['errcode'] == 0:
            ...
            print (f'账号【{str(i+1)}】获取用户信息失败，账号异常:{result}')
            return False
        ...
        if result['errcode'] == 0:
            print(f"账号【{str(i+1)}】今日积分: {result['data']['day_gold']} 已阅读: {result['data']['day_read']}篇 剩余: {result['data']['remain_read']}篇")
        else:
            print (f'账号【{str(i+1)}】获取用户信息失败，账号异常:{result}')
            return False
...
def do_read(i,ck):
    ...
    while True:
            ...
            if result['errcode'] == 0:
                ...
                if biz == []:
                    print(f'账号【{str(i+1)}】未找到BIZ,重新获取')
                    print(link)
                    continue
                else:
                    biz = biz[0]
                ...
                if biz in check_list:
                    ...
                    if check == True:
                        ...
                        if r_result['errcode'] == 0:
                            ...
                            print(f"账号【{str(i+1)}】检测异常:{r_result}")
                            break
                    else:
                ...
                if result['msg'] == "任务重复" or result['msg'] == "任务超时":
                    print(f"账号【{str(i+1)}】阅读失败: {result['msg']}重新获取文章")
                else:
                    print (f"账号【{str(i+1)}】阅读提醒: {result['msg']}")
                    break
    ss.close
