...
def user_info(i,ck):
    ...
    result = requests.post(domain+'/web-read/read-info',data=data,headers=headers).json()
    if result['code'] == 201:
        print(f"账号【{str(i+1)}】账号异常请检查该账号ck是否正确!")
        return False
    else:
        print(f"账号【{str(i+1)}】钱包余额: {result['data']['walletMoney']}")


def do_read(i,ck):
    print(f"账号【{str(i+1)}】开始阅读")
    for i in range(30):
        ...
        if result['code']==200:
            ...
            print(f"账号【{str(i+1)}】阅读失败:{result['msg']}")
            break
    print(f"账号【{str(i+1)}】阅读任务完成")


def get_money(i,ck):
    tdata = f'readUserId={ck["uid"]}&sign={ck["sign"]}'
    result = requests.post(domain+'/web-read/cash-out',data=tdata,headers=headers).json()
    if result['code'] == 200:
        print(f"账号【{str(i+1)}】提现成功 {result['data']['topTitle']}")
    else:
        print(f"账号【{str(i+1)}】提现失败 {result['msg']}")
...
███████╗██║██║ ╚████║██╔╝ ██╗██║     ██╔╝ ██╗███████╗   ██║   ██████╔╝
╚══════╝╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝╚═╝     ╚═╝  ╚═╝╚══════╝   ╚═╝   ╚═════╝ 
    项目:小啄阅读(几秒到低保0.3)       BY-林夕       Verion: 0.0.2(并发)
